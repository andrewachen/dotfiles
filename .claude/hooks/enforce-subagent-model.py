    ...
    if tool_name in ("Agent", "Task"):
        ...
        if not tool_input.get("model"):
            deny(
                ...
                "inherits the session model. Choose the cheapest option that can do "
                "the job — the failure mode is reflexively reaching for the most "
                "capable model:\n"
                "  - A named DELEGATE agent for anything substantial (see the "
                "delegation policy in the session's steering). Prefer a named "
                "delegate over a tier whenever one matches the work.\n"
                "  - \"haiku\": haiku-tier work ONLY — mechanical, fully-specified, "
                "1-2 files, no integration risk, no judgment required. This is not "
                "the default; most substantive work belongs on a delegate.\n"
                "  - \"sonnet\": genuine code comprehension or judgment a smaller "
                "model would get wrong — writing non-trivial new code, reviewing a "
                "diff for correctness, a cross-file refactor, debugging a "
                "non-localized cause, reasoning about unfamiliar code to answer a "
                "question. Name why the cheaper option is insufficient before "
                "choosing it; but don't starve a task that needs it — a run that "
                "botches comprehension work just gets redone higher, costing more "
                "than starting here.\n"
                "  - \"opus\": hard reasoning, ambiguous design, subtle debugging.\n"
                "If you truly want the session model, pass `model: \"inherit\"` to "
                "say so explicitly."
            )
        sys.exit(0)
    ...
    if tool_name == "Workflow":
        ...
        if offenders:
            ...
            deny(
                ...
                "so each spawned subagent silently inherits the session model. Add "
                "a `model:` to each, choosing the cheapest option that can do the "
                "job:\n"
                "  - 'haiku': haiku-tier work only — mechanical, fully-specified, "
                "search, reading a known file, a localized edit, summarizing. Not "
                "the default.\n"
                "  - 'sonnet': genuine code comprehension or judgment a smaller "
                "model would get wrong — writing non-trivial code, reviewing a "
                "diff, a cross-file refactor, debugging a non-localized cause. "
                "Name why the cheaper option won't do; a botched run just gets "
                "redone higher.\n"
                "  - 'opus': hard reasoning, ambiguous design, subtle debugging.\n"
